...
import { Loader2, Search, MoreVertical, Trash2, Ban, CheckCircle, Shield } from "lucide-react";
...
export default function AdminDashboard() {
    ...
    const [actionLoading, setActionLoading] = useState<string | null>(null); // storing userId being acted upon
    ...
    useEffect(() => {
        loadUsers();
        // Poll for online status updates every 30s
    ...
    const handleDelete = (userId: string) => {
        setModalConfig({
            ...
            type: "danger",
            confirmText: "Delete User", // Note: Passing this via prop in render
            onConfirm: async () => {
                setActionLoading(userId);
                try {
                    const token = localStorage.getItem('token');
        ...
        <div className="space-y-8">
            ...
            <header className="flex justify-between items-center">
                <div>
                    ...
                    <p className="text-zinc-500 font-medium">Overview of all registered members</p>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="glass-card p-6 border-emerald-500/20 bg-emerald-500/5">
                    <p className="text-emerald-500 text-xs font-bold uppercase tracking-widest mb-2">Online Now</p>
                    <h3 className="text-3xl font-black text-emerald-400">
                        {users.filter(u => isOnline(u.lastActive)).length}
                    </h3>
                </div>
                <div className="glass-card p-6 border-red-500/20 bg-red-500/5">
                    <p className="text-red-500 text-xs font-bold uppercase tracking-widest mb-2">Banned</p>
                    <h3 className="text-3xl font-black text-red-400">
                        {users.filter(u => u.isLoginEnabled === false).length}
                    </h3>
                </div>
            </div>
